...
<?php
	...
	if(isset($_REQUEST['libro']) && 
		...
		$datiRecensione = 
			$db->query("SELECT Id,Testo,Valutazione FROM Recensione WHERE Libro =". $codice);
		...
		if($datiLibro->num_rows > 0) {
			...
			$autoreArray = $db->query("SELECT Nome,Cognome,Id FROM Scrittore 
			...
			$replaceHeader=array($errore,$datiL['ISBN'],$datiL['Titolo'],$autore['Id']
